    ...
    if model_params['z3']:
        ...
        while max_path>=instance_data['lower_bound']:
            ...
            if result=='unsat':
                ...
                break
            # if time.time()-execTime>params['timeout']+0.5:
            #     if verbose:print('skipping last solution found: time exceeded the limit')
            #     break
            obj=max(distances)
            max_path=obj-1  
...
def launch_cvc5(instance_file, params, model_params, verbose):
    ...
    while max_path >= instance_data['lower_bound'] and remaining_time > 0:
        ...
        if p.is_alive():
            ...
            max_path = obj - 1
            if verbose:
                print(f'Found solution with objective {obj}: {solution}')
            best_model = model
            if max_path < instance_data['lower_bound']:
                if verbose:
