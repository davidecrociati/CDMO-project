...
def evaluate(model, model_variables, num_couriers, num_items):
    ...
    print(loads,distances_traveled,item_resp,successors)
    for row in stops:
        print(row)
...
def solve_z3_model(params:dict, data):
    ...
    if 'timeout' in params:
        pars['timeout']=math.floor(pars['timeout']-(time.time()-aux_time))*1000
    optimizer.set(**pars)
    try:
        ...
        if status == sat: # Optimal
            ...
            if results:
                return tolist(results [0]), max(results[1]),True
            else:
                return [], 'N/A'
