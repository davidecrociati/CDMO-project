...
# Function to define the bin packing problem in Z3 using Boolean variables
def bin_packing_capa(c, w):
    num_items = len(w)
    num_bins = len(c)
    ...
    for i in range(num_items):
        s.add(Sum([If(bin_vars[i][b], 1, 0) for b in range(num_bins)]) == 1)

    # Constraint: the capacity of each bin must not be exceeded
    for b in range(num_bins):
        s.add(Sum([If(bin_vars[i][b], w[i], 0) for i in range(num_items)]) <= c[b])
...
solver, bin_vars = bin_packing_capa(c, w)
